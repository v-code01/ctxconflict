...
echo "== 2/6 mypy --strict =="; MYPYPATH=src $M --strict src tools/run_gen.py tools/analyze.py tools/verify.py || fail mypy; echo "   ok"
echo "== 3/6 pytest =="; $PY -m pytest -q || fail pytest
echo "== 4/6 pure-ASCII =="
bad=$(LC_ALL=C grep -rlP '[^\x00-\x7F]' src tests tools scripts README.md claims.toml REVIEW.md PREREG.md bench_results docs 2>/dev/null || true)
...
leak=$(grep -rniIE --exclude-dir=__pycache__ --exclude-dir=.venv "$pat" \
  src tests tools README.md claims.toml REVIEW.md PREREG.md reproduce.sh docs 2>/dev/null || true)
